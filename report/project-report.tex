...
The \texttt{opts} stores file server options
The \texttt{FileServer} maintains connection with the available peers via the \texttt{peers}, that stores peer addresses and peer connections as key-value pairs. It uses \texttt{store} for local file storage and management.
It keeps track of files that are actively being shared with the \texttt{activeFiles} map. This allows it to quickly determine if it has the particular chunk requested by a peer.

The \texttt{responseHandlers} slice contains callback functions that are responsible for handling different types of peer messages for ongoing downloads.

The \texttt{FileServer} maintains a different peer map \texttt{udpPeers} for tracking UDP-capable peers. Major reasons for this inclusion are for Network Address Translation-Traversal (NAT-T; more on it later) and generally higher speed transfers compared to TCP. This way it can prioritise faster UDP transfers when possible, falling back to TCP for larger transfers or for peers that do not support UDP.

Some of the key operations the \texttt{FileServer} provides are:
\begin{itemize}
	\item \textbf{ShareFile:} Makes a local file available for peers to download.
	\item \textbf{DownloadFile:} Initiates the file download from peers in the network.
	\item \textbf{handleChunkRequest:} Responds to file chunk requests from peers in the network.
	\item \textbf{downloadChunk:} Retrieves a specific chunk from available peers.
\end{itemize}

The most important part of the \texttt{FileServer} is how it facilitates sharing and especially the download process. It implements a sophisticated download mechanism that can download different chunks from different peers maximising throughput which is co-ordinated by the \texttt{Piece Manager}.

\subsection{Transport Layer}
The system implements a dual-protocol transport system with both TCP and UDP. The TCP implementation was mostly taken from an already well established project on GitHub called \texttt{foreverstore} \cite{anthdm-foreverstore}. UDP transport implementation was also inspired by \texttt{foreverstore}. The transport layer abstracts the communication details from the higher level components.

The \texttt{TCPTransport} structure in \texttt{tcp\_transport.go} ensures reliable TCP connections between peers:

\begin{lstlisting}[language=Go, caption=Transport Implementation]
...
The \texttt{UDPTransport} structure in \texttt{udp\_transport.go} implements UDP with, although incomplete, support for NAT-Traversal.
...
type UDPTransport struct {
	...
	udpResponseHandlers sync.Map
	udpRequestsMu       sync.Mutex
	udpRequests         map[string]UDPRequest
